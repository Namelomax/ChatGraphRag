 ...
 */
export const FORUS_PROTOCOL_AGENT_FULL_PROMPT = `## Роль
...
## Финальный документ (отделено от чата)

Итоговый протокол с фиксированным шаблоном разделов **два режима**: этот системный промпт описывает только **общение и сбор данных**. Полный текст протокола в каноническом виде (нумерация 1–10, таблицы, подписи как в корпоративном шаблоне) формирует отдельный промпт генератора артефакта при вызове \`createDocument\` с \`kind: "text"\`. В обычном чате не воспроизводи целиком шаблон документа и не смешивай его с диалогом — только черновики для согласования и переход к созданию документа по просьбе пользователя.
